module core (input logic clk_i, 
    ...
    logic edge_trig, next_trig;
    edge_detect ed0 (.clk_i(clk_i), .sig_i(next_i), .edge_o(next_trig));
    edge_detect ed1 (.clk_i(clk_i), .sig_i(ext_int_i), .edge_o(edge_trig));
    ...
    pc  pc0  (.clk_i(next_trig), .halt_i(halt_o||wfi_core), .n_rst_i(n_rst_i), .mode_i(mode_pc), .data_i(immediate), .addr_o(prog_addr));
